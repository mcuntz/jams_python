...
def signature2plot(handle, dx=None, dy=None, name=None,
    ...
    """
        Write a copyright notice on a plot:
            (C) name\nCHS/UFZ Leipzig, YYYY
        or if no name given
            (C) CHS/UFZ Leipzig, YYYY
    ...
    if iname is not None:
        if usetex:
            ...
            if '&' in s1:
                s1 = s1.replace('&', '\&')
            s2 = r'$\mathrm{CHS/UFZ\, Leipzig,\, '+year+'}$'
        else:
            s1 = r'$\copyright$ '+iname.strip()
            s2 = r'CHS/UFZ Leipzig, '+year
    else:
        if usetex:
            s1 = r'$\mathrm{\copyright\, CHS/UFZ\, Leipzig,\, '+year+'}$'
            s2 = None
        else:
            s1 = r'$\copyright$ CHS/UFZ Leipzig, '+year
            s2 = None
