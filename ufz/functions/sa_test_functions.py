...
"""
    ...
    saltelli              Example on page 179 of Saltelli et al. (2008)
    ...
    g                     g-function attributed to Sobol' (1990, 1993), given by Saltelli et al. (2008)
...
__all__ = ['B', 'g', 'Gstar', 'K', 'morris', 'oakley_ohagan', 'saltelli']
...
# def B(para):
#     ''' B example, Saltelli et al. (2010) Comp. Phys. Comm., 181, p. 259-270'''
#     # Parameter sets are assumed to be in following ordering: (x_1, x_2, ..., X_m, w_1, w_2, ..., w_m)
#     para = np.array(para)
#     if para.ndim == 1:
#         isone = True
#         ipara = para[:,np.newaxis]
#     else:
#         isone = False
#         ipara = para

#     m  = ipara.shape[0]//2
#     rr = np.random.randn(ipara.shape) * para
#     y  = np.sum(rr[:m,:]*rr[m:,:], axis=1)
...
def B(para):
    ...
    m  = ipara.shape[0]//2
    y  = np.sum(ipara[:m,:]*ipara[m:,:], axis=0)
...
def Gstar(para, alpha, delta, a):
    ...
    yi = ((1.+ialpha)*np.abs(2.*(ipara+idelta-np.trunc(ipara+idelta))-1.)**ialpha+ia)/(1.+ia)
    y  = np.prod(yi, axis=0)
...
def K(para):
    ...
    npara = ipara.shape[0]
    for ii in range(1,npara):
        ipara[ii,:] = ipara[ii-1,:] * ipara[ii,:]
    for ii in range(npara):
        ipara[ii,:] = (-1)**(ii+1) * ipara[ii,:]
    y = ipara.sum(axis=0)
...
def morris(X, beta0, beta1, beta2, beta3, beta4):
    ...
    om = 2.*(X - 0.5)
    ii = np.array((2, 4, 6))
    om[ii] = 2.*(1.1*X[ii]/(X[ii]+0.1) - 0.5)
    
    out = beta0
    for i in range(0,20):
        out += beta1[i]*om[i]
        for j in range(i+1,20):
            out += beta2[i,j]*om[i]*om[j]
            for l in range(j+1,20):
                out += beta3[i,j,l]*om[i]*om[j]*om[l]
                for s in range(l+1,20):
                    out += beta4[i,j,l,s]*om[i]*om[j]*om[l]*om[s]
...
def oakley_ohagan(X):
    ...
    out  = np.dot(a1,X)
    out += np.dot(a2,np.sin(X))
    out += np.dot(a3,np.cos(X))
    for i in range(X.shape[1]):
        out[i] += np.dot(X[:,i].T,np.dot(M,X[:,i]))
    return out.squeeze()

# -----------------------------------------------------------

def saltelli(para, a, b):
    ''' Saltelli example, Primer p. 179'''
    # Model output for given parameter set(s) is returned
    # para: dim1 = # of parameters = 3
    #       dim2 = # of parameter sets
    para = np.array(para)
    if para.ndim == 1:
        isone = True
    ...
    y = np.sin(ipara[0,:])  + a*(np.sin(ipara[1,:]))**2  + b*ipara[2,:]**4  * np.sin(ipara[0,:])
