...
def create_network(nnodes, nedges, maxdegree=None, maxoutdegree=None, maxindegree=None, reverse=False, verbose=False):
    """
        ...
        along with the JAMS project (cf. gpl.txt and lgpl.txt).
    ...
    while (not found_network):
        ...
        cedge = 0
        while cedge < nedges:
            rand = random.randrange(npos_edges)  # integer random number of interval [0,npos_edges)
            ...
            # check if this element is already set to "1", if yes try another one
        ...
        if (degrees_satisified):
            ...
            nodes = G.nodes()
            if verbose: print('Added nodes: ',nodes)
...
def plot_network(G, fname_plot):
    """
        This functions plots a graph using either graphviz if available or standard matplotlib.
        Plots with graphviz are much more fancy since it reduces the number of intersections of
        plotted edges and draws round edges unstead of only straight lines.
    ...
    color   = [(0.237736, 0.340215, 0.575113), # get_brewer('dark_rainbow_256', rgb=True)[0]   # blue
               (0.404682, 0.548016, 0.244594), # get_brewer('dark_rainbow_256', rgb=True)[100] # green
               (0.786572, 0.753215, 0.297951)] # get_brewer('dark_rainbow_256', rgb=True)[150] # yellow
    sources = source_nodes(G)
    sinks   = sink_nodes(G)
    nodes = G.nodes()
    edges = G.edges()
    ...
    try:
        ...
        A.add_edges_from(G.edges(),color='grey',arrowhead='vee')
        ...
        for ii in nodes:
            ...
            if ii in sinks:
                cc          = color[2]
                cc_hex_fill = "#%2x%2x%2x%2x" % (np.int(cc[0]*255) , np.int(cc[1]*255) , np.int(cc[2]*255) , np.int(alpha_fill*255))
                cc_hex_bndr = "#%2x%2x%2x%2x" % (np.int(cc[0]*255) , np.int(cc[1]*255) , np.int(cc[2]*255) , np.int(alpha_bndr*255))
            elif ii in sources:
                cc          = color[0]
                cc_hex_fill = "#%2x%2x%2x%2x" % (np.int(cc[0]*255) , np.int(cc[1]*255) , np.int(cc[2]*255) , np.int(alpha_fill*255))
                cc_hex_bndr = "#%2x%2x%2x%2x" % (np.int(cc[0]*255) , np.int(cc[1]*255) , np.int(cc[2]*255) , np.int(alpha_bndr*255))
            ...
            # plot node
            A.add_node(ii,color=cc_hex_bndr,fillcolor=cc_hex_fill, style='filled')
