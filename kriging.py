...
def kriging(x, y, z, semi_mod, semi_popt, xnew=None, ynew=None, plot=False,
    ...
    """
        ...
        HISTORY:
        Written,  AP, Nov 2012
        Modified, MC, Nov 2012 - default plot=False and silent=True, removed xtick.major.width
    """
    ...
    if masked:
        ...
        start = time.time()
        hull_points = convex_hull(np.vstack((x,y)), smidgen=0.0075)
        stop = time.time()
        if not silent:
    ...
    if xnew != None:
        ...
        #lamnew = np.empty((xnew_v.size, x.size))
        if masked:
            mask = np.empty_like(xnew_v, dtype=np.int)
    ...
    if eop != None:
        ...
        #eoplam = np.empty((eopx.size, x.size))
        if masked:
...
if __name__ == '__main__':
    ...
    #                                    plot=False,masked=False,eop=None)
    # print znew[0]
    # #[ 3.57640726  3.75752385  3.91100552  3.93576323  3.88277335  3.82858082
    # #  3.79098281  3.75840203  3.70984856  3.61284328  3.40724353  2.98163259
    # #  2.16512767  2.36768411  2.46051345  2.79909103  3.30483349  3.81621237
    # #  4.29557546  4.71373854  4.91395225  4.76644181  4.47411423  4.23511553]
    ...
    #                        silent=True,eop=poi)
    # print eopz
    # #[ 6.40416991  1.68017649  3.16804585  1.26269999  4.6314843   6.53340616
    # #  2.24473939  2.25902761  2.99659575  2.11443535]
